      ...
      <xsl:when test="descendant::*[x:is-block(.)]">
        ...
        <xsl:for-each-group select="node()" group-adjacent="x:is-block(.)">
          <xsl:choose>
            ...
            <xsl:when
              test="count(current-group()) eq 1 and current-group()/self::text() and not(normalize-space(current-group())) or (current-group()/local-name()='draft-comment')"/>
            <xsl:otherwise>
    ...
    <xsl:copy>
      ...
      <xsl:for-each-group select="node()" group-adjacent="x:is-block(.)">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:choose>
              <xsl:when test="current-group()/local-name()='draft-comment'">
                <!-- <xsl:variable name="current-nodes" select="current-group()"/>-->
                <xsl:variable name="nodes-without-comments">
                  <xsl:for-each select="current-group()">
                    <xsl:if test="local-name()!='draft-comment'">
                      <xsl:copy>
                ...
                <xsl:if test="$nodes-without-comments/node()">
                  <p class="- topic/p ">
                    <xsl:apply-templates select="$nodes-without-comments" mode="flatten"/>
                  </p>
                </xsl:if>
              </xsl:when>
              <xsl:otherwise>
